...
#Clearing database - couldnt get working. 


# conn = Connection(username="TUTyq8bcf5vktpkd4m27x7qp", password="TUT7p2yidv7tfqwv26lheeir9")
# db = conn["TUTzapb7o5xfp870yuiakd6ao"]

# collections = db.collections
# for collection_name in collections:
#     if not collection_name.startswith('_'):  # Exclude system collections
...
# collections = [col for col in movie_rec_db.collections() if not col.name.startswith('_')]
...
#    collection.truncate() 
# for collection in collections:
#     query = f"FOR doc IN {collection.name} REMOVE doc IN {collection.name}"
#     movie_rec_db.aql.execute(query)
...
torch.save(title_emb, "movie_embeddings.pt")
...
torch.save(movie_x, "movie_embeddings_concat.pt")
...
import matplotlib.pyplot as plt
from sklearn.manifold import TSNE 
def visualize_embeddings(model, data, movie_genres):
    movie_embeddings = model.encoder(data.x_dict, data.edge_index_dict)['movie'].detach().cpu().numpy()
    embeddings_2d = TSNE(n_components=2).fit_transform(movie_embeddings)
    unique_genres = set(movie_genres)
    genre_to_idx = {genre: i for i, genre in enumerate(unique_genres)}
    fig, ax = plt.subplots()
    colors = plt.cm.get_cmap('tab10', len(unique_genres))  
    for i, embedding in enumerate(embeddings_2d):
        genre = movie_genres[i]
        idx = genre_to_idx[genre]  # Map genre to index
        ax.scatter(embedding[0], embedding[1], color=colors(idx), label=genre) 
    ax.legend()
...
movie_genres = [doc['genres'][0] for doc in movie_rec_db.aql.execute('FOR doc IN Movie RETURN doc') if 'genres' in doc and doc['genres']]   
visualize_embeddings(model, data, movie_genres) 
